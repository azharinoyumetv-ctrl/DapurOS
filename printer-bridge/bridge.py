#!/usr/bin/env python3
"""DapurOS Printer Bridge -- run this on the same PC as your network thermal printer.

Why this exists: DapurOS runs in your browser and its backend runs in the cloud. Neither
...
Ported from GerainaOS/printer-bridge/bridge.py (identical logic — this script is generic,
no Geraina/DapurOS-specific branding baked into the protocol itself).
...
BRIDGE_PORT = int(os.environ.get("DAPUROS_BRIDGE_PORT", "9899"))
PRINT_TIMEOUT_SECONDS = 5

# Only allow requests from DapurOS's own origins -- this bridge accepts a target IP/port
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/health":
            ...
            self.end_headers()
            self.wfile.write(json.dumps({"ok": True, "service": "dapuros-printer-bridge"}).encode())
            return
...
def main():
    ...
    print(f"DapurOS Printer Bridge listening on http://127.0.0.1:{BRIDGE_PORT}")
