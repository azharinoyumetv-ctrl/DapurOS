...
HISTORY: sebelumnya file ini mengirim lewat gateway pihak ketiga (Fonnte/Wablas) -- bukan
Meta resmi. Dimigrasikan ke Meta Cloud API mengikuti migrasi yang sama di GerainaOS, supaya
kedua produk konsisten dan tak bergantung pada API gateway pihak ketiga yang tak terverifikasi.
...
SYNC: KEEP IN SYNC dengan GerainaOS/backend/whatsapp_client.py.
"""
import os
import httpx

# Verify this against developers.facebook.com/graph-api/changelog before relying on it
...
# approval -- same status as GerainaOS's. Utility-category, text-only, no-button templates like
# these are typically Meta's fastest approval tier; submit via Meta Business Manager before
# relying on these to actually send anything.
...
async def send_meta_message(cfg: dict, target: str, template_name: str, params: list, lang: str = "id") -> dict:
    ...
    Business Manager sebelum fungsi ini bisa benar-benar mengirim apa pun.
    """
