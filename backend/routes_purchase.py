...
async def create_purchase_order(payload: PurchaseOrderCreate, user: dict = Depends(get_current_user)):
    ...
    # Diinisiasi toko -> wajib template. TEMPLATE_PO_NAME_DEFAULT belum disetujui Meta -- lihat
    # TEMPLATE_PO_BODY_ID di whatsapp_client.py untuk teks yang perlu diajukan. Migrated off the
    # old Fonnte/Wablas gateway client onto Meta Cloud API (mirrors GerainaOS).
    try:
...
async def update_supplier_invoice(invoice_id: str, payload: SupplierInvoiceUpdate, user: dict = Depends(get_current_user)):
    ...
    and requires invoice_no/po_no/amount/due_date that were never sent, so it 422'd. There
    was also no .catch(), so the failure was silent (same bug pattern as the debt
    receivable/payable settle buttons, fixed earlier this session). Confirmed this bug exists
    identically on GerainaOS too -- fixing there as well, not just here.
    """
    db = get_db()
