...
SYNC: KEEP IN SYNC with GerainaOS/backend/plan_limits.py -- two separate backends share the
same numeric caps from the pricing repackaging spec, so any change here must be mirrored
there (DapurOS additionally has max_tables for the table/floor-plan resource, which
GerainaOS's tiers don't carry).

check_capacity() blocks creating one more of a resource (staff, products, tables) once a
store's plan-based cap is reached. check_outlet_capacity() does the same for branches,
accounting for the store's own base location counting as outlet #1.
...
async def _active_addon_counts(db, store_id: str) -> dict:
    ...
    a manual step (no subscription payment gateway wired to this app's own billing)."""
    counts = {}
...
def _addon_bonus(counts: dict, field: str) -> int:
    ...
    routes_pricing.py: extra_device = +1 device each; extra_outlet bundles +1 outlet, +8
    devices, +25 employee seats each ("termasuk 8 device & 25 akun karyawan") -- DapurOS's
    numbers differ from GerainaOS's (5/15) since each app's outlet bundle mirrors that app's
    own Pro-tier device/employee caps."""
    extra_device = counts.get("extra_device", 0)
    extra_outlet = counts.get("extra_outlet", 0)
    if field == "max_devices":
        return extra_device + extra_outlet * 8
    if field == "max_employees":
        return extra_outlet * 25
    return 0
