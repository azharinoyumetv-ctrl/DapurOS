...
Ported from GerainaOS/backend/routes_edc.py. One DapurOS-specific difference: the config
lives under payments_config.debit_card (DapurOS's existing Settings > Pembayaran > Kartu
Debit tab), not a separate "edc" key -- DapurOS already had a debit-card config shape before
this integration existed, so this reuses it instead of introducing a second, overlapping key.

Note: DapurOS's POS.jsx checkout for card/EDC/debit/credit uses a manual cashier-confirms-
after-the-physical-terminal-approves flow, which is the correct default for the overwhelming
majority of merchants whose EDC machine has no software link to the till at all. This endpoint
is NOT wired into that checkout path -- it exists so that a merchant who does eventually get a
software-integrated terminal has a real, TOCTOU-safe charge endpoint ready to call, without
requiring a rewrite of the order flow (see get_edc_provider in edc_provider.py)."""
...
async def list_edc_providers(user: dict = Depends(get_current_user)):
    """Bank options available in Settings > Pembayaran > Kartu Debit. All currently
    unconfigured -- see edc_provider.py docstring for why (each needs its own SDK + bank
    certification)."""
...
async def charge_order_via_edc(order_id: str, user: dict = Depends(get_current_user)):
    ...
    edc_cfg = (payments_config or {}).get("debit_card") or {}
    if not edc_cfg.get("is_active") or not edc_cfg.get("provider"):
        ...
        # GerainaOS production (TestSprite caught it as "invalid or incomplete response to
        # Cloudflare") -- same fix applied here up front.
        raise HTTPException(status_code=400, detail="EDC belum dikonfigurasi. Atur bank EDC di Pengaturan > Pembayaran > Kartu Debit.")
