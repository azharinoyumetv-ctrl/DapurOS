...
def _norm_module(m: str) -> str:
    m = (m or "dapuros").lower()
    return m if m in VALID_MODULES else "dapuros"
...
async def register(payload: UserRegister, x_dagangos_module: str = Header(default="dapuros", alias="X-DagangOS-Module")):
    ...
    user_doc = {
        ...
        "role": "Owner",
        "store_id": store_doc["id"],          # toko utama / terakhir aktif
        "store_name": payload.store_name,
        "last_active_store_id": store_doc["id"],
        "plan": "trial",                       # langganan per-AKUN, mencakup semua toko
