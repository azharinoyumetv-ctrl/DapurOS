...
integrations.xendit.webhook_token (BYO, like the Xendit credentials themselves).

HISTORY: this used to check the inbound x-callback-token against a single global
XENDIT_WEBHOOK_TOKEN env var. That was already inconsistent with the rest of this session's
Xendit work -- routes_orders.py/xendit_client.py/routes_settings.py were all migrated to BYO
per-store credentials (each store pastes its own secret_key + webhook_token into Pengaturan >
Integrasi), but the webhook RECEIVER was never updated to match: with a global token check, at
most one store's webhook token could ever match (whichever happened to equal the env var), so
every other store's Xendit payment callbacks would be silently rejected with 401 and never
update to "paid". Fixed to look up the tenant by their own saved webhook_token, mirroring
GerainaOS's routes_webhooks.py (and DOKU's already-correct per-tenant pattern in this same file).
Also kept DapurOS's own order-lookup-first + amount-verification hardening (see _process below)
which GerainaOS's simpler version doesn't have -- worth porting back there separately.
"""
import hashlib
import hmac
import logging
...
# Xendit callback-token verification follows the same per-tenant pattern -- see xendit_webhook
# below.
...
async def _process(payload: dict, store_id: str):
    ...
    # token belongs to, so we know which store it belongs to and what it's actually worth. This
    # lets us verify the amount before ever flipping to "paid", so a webhook meant for one
    # store's payment can't silently mark a different store's order as paid.
...
async def xendit_webhook(req: Request, background: BackgroundTasks):
    """Single shared URL for every tenant, same shape as the DOKU/WhatsApp handlers below: the
    inbound x-callback-token tells us which store's own Xendit account this call belongs to
    (each store pastes ITS OWN token into Pengaturan > Integrasi and configures ITS OWN Xendit
    dashboard to call back here with it) -- nothing is trusted or acted on until that lookup
    succeeds."""
...
async def simulate_midtrans_webhook(payload: dict, req: Request, store_id: str = ""):
    """Dev-only Midtrans webhook simulator (no token required). store_id now required to match
    _process's per-tenant signature above -- previously called _process(mapped) with a single
    arg, which would have raised a TypeError the moment _process gained its store_id parameter."""
    if (os.environ.get("ALLOW_WEBHOOK_SIMULATE", "false").lower() not in ("1", "true", "yes")):
        raise HTTPException(status_code=403, detail="Simulator disabled")
    if not store_id:
        raise HTTPException(status_code=400, detail="store_id wajib diisi untuk simulator")
    ...
    await _process(mapped, store_id)
    return {"ok": True}


@router.post("/stripe/simulate")
async def simulate_stripe_webhook(payload: dict, req: Request, store_id: str = ""):
    """Dev-only Stripe webhook simulator (no token required). Same store_id fix as Midtrans
    above."""
    if (os.environ.get("ALLOW_WEBHOOK_SIMULATE", "false").lower() not in ("1", "true", "yes")):
        raise HTTPException(status_code=403, detail="Simulator disabled")
...
# verification passes. Ported from GerainaOS/backend/routes_webhooks.py.
...
async def simulate_whatsapp_webhook(payload: dict, req: Request, store_id: str = ""):
    """Dev-only WhatsApp inbound webhook simulator (no Meta signature required). Mirrors
    _process_whatsapp_inbound's real routing/storage shape (same entry[].changes[].value.
    metadata.phone_number_id envelope Meta actually sends), but skips the HMAC signature check
    and takes store_id explicitly rather than re-deriving the tenant from phone_number_id, since
    a simulator is run by someone who already knows which store they're testing."""
    if (os.environ.get("ALLOW_WEBHOOK_SIMULATE", "false").lower() not in ("1", "true", "yes")):
        raise HTTPException(status_code=403, detail="Simulator disabled")
    if not store_id:
        raise HTTPException(status_code=400, detail="store_id wajib diisi untuk simulator")
...
async def list_whatsapp_inbound(user: dict = Depends(get_current_user)):
    """Authenticated, store-scoped read of this store's own received inbound WhatsApp messages
    (real + simulated) -- lets the Integrasi simulator panel show that a message actually landed
    instead of firing a POST into the void with no way to verify it."""
    db = get_db()
...
async def doku_webhook(req: Request, background: BackgroundTasks):
    """DOKU always expects a 200 with a specific ack body -- verification failures are logged
    internally (via _process_doku silently discarding), not surfaced as HTTP errors, since DOKU
    will otherwise retry-storm an endpoint that 4xx/5xxs it."""
    raw = await req.body()
...
async def simulate_doku_webhook(payload: dict, req: Request, store_id: str = ""):
    """Dev-only DOKU webhook simulator (no signature required). Requires an explicit store_id
    (same contract as the real handler's Client-Id tenant resolution above) -- matching purely
    by order_no with no store scope at all would be the same cross-tenant collision class
    already fixed for /xendit's handlers, even though order_no now also carries a store-hash
    suffix (see next_order_no in database.py) that makes a same-day collision practically
