...
Ported from GerainaOS/backend/edc_provider.py (identical -- this module is generic, no
Geraina/DapurOS-specific branding baked in).
...
Note for DapurOS specifically: most merchants' EDC machines are NOT software-integrated at all
(the till software has no link to the bank's terminal) -- the cashier swipes/taps on the
physical machine and then manually confirms in the POS once it approves. DapurOS's POS.jsx
already implements that manual-confirm flow and it remains the correct default path. This
module exists so that IF a merchant ever gets a software-integrated terminal, wiring in that
bank's real SDK here is a self-contained change -- nothing else in the order flow needs to
change (see get_edc_provider below).
