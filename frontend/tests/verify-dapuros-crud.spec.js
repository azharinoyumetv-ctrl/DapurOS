// CRUD menyeluruh DapurOS dalam SATU run:
//  A) UI CRUD (form -> API -> list) untuk form inline yang umum dipakai kasir.
//  B) API CRUD (create/read/delete) untuk SEMUA entitas master-data, termasuk yang
//     form-nya modal (produk, staf, cabang, membership, bahan).
// Semua hasil dikumpulkan; satu run melaporkan OK/FAIL tiap entitas.
const { test, expect } = require("@playwright/test");

const STAMP = Date.now();
const EMAIL = `e2e.crud.${STAMP}@gmail.com`;
const PASSWORD = "Verify123!";
const STORE = `E2E CRUD ${STAMP}`;

// ---- A) Form inline (UI): input nama (+ tambahan) -> submit -> tabel Edit/Hapus ----
const UI_ENTITIES = [
  { label: "UI Kategori", route: "/dapuros/app/products/categories", nameLoc: '[data-testid="category-name-input"]', submit: "category-submit", list: "categories-list", extra: [] },
  { label: "UI Merek", route: "/dapuros/app/products/brands", nameLoc: '[data-testid="brand-name-input"]', submit: "brand-submit", list: "brands-list", extra: [] },
  { label: "UI Satuan", route: "/dapuros/app/products/units", nameLoc: '[data-testid="unit-name-input"]', submit: "unit-submit", list: "units-list", extra: [['[data-testid="unit-symbol-input"]', "e2e"]] },
  { label: "UI Pemasok", route: "/dapuros/app/suppliers", nameLoc: '[data-testid="supplier-form"] input[placeholder="Nama PT / Toko Pemasok"]', submit: "supplier-submit", list: "suppliers-list", extra: [] },
  { label: "UI Pelanggan", route: "/dapuros/app/customers", nameLoc: '[data-testid="customer-form"] input[placeholder="Contoh: Budi Gunawan"]', submit: "customer-submit", list: "customers-list", extra: [] },
];

// ---- B) API CRUD: base endpoint + builder payload + apakah punya DELETE ----
const API_ENTITIES = [
  ...
  { label: "API Bahan(BOM)", base: "/api/ingredients", deletable: true, mk: (n) => ({ name: n, unit: "gram", stock: 100, safety_stock: 10 }) },
...
test("DapurOS — CRUD menyeluruh (UI + API)", async ({ page, request }) => {
