...
export default function Reports() {
  ...
  const renderReport = () => {
    switch (type) {
      case "sales": {
        const trendData = (salesTrend || []).map((d) => ({ day: d.day, sales: d.sales }));
        return (
          <div className="space-y-6">
            <div className="card-surface p-6 h-80">
              <h3 className="font-display font-bold text-sm mb-4">Tren Penjualan 7 Hari Terakhir</h3>
              {salesTrend === null ? (
                <p className="text-sm text-[hsl(var(--muted))]">Memuat data penjualan...</p>
              ) : (
