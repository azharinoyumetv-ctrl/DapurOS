  ...
  return (
    <div className="p-8 space-y-6" data-testid="suppliers-page">
      <div>
        ...
        <h1 className="font-display text-3xl font-bold mt-1">Database Pemasok (Supplier)</h1>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        <form onSubmit={handleSubmit} className="col-span-12 lg:col-span-4 card-surface p-6 space-y-4" data-testid="supplier-form">
          <h2 className="font-display font-bold text-lg">{editingId ? "Edit Pemasok" : "Tambah Pemasok"}</h2>
          <div className="flex flex-col space-y-1">
            <label className="text-xs font-semibold text-[hsl(var(--muted))] uppercase">Nama Pemasok</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="Nama PT / Toko Pemasok"
          ...
          <button type="submit" className="btn-primary w-full py-2 flex items-center justify-center gap-2" data-testid="supplier-submit">
            {editingId ? "Simpan Perubahan" : <><Plus size={16} /> Tambah Pemasok</>}
          </button>
          {editingId && (
            <button
              type="button"
        ...
        <div className="col-span-12 lg:col-span-8 card-surface p-6" data-testid="suppliers-list">
          <h2 className="font-display font-bold text-lg mb-4">Daftar Pemasok</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-[hsl(var(--border))]">
                  <th className="py-3 font-semibold text-[hsl(var(--muted))]">Nama Pemasok</th>
              ...
              <tbody className="divide-y divide-[hsl(var(--border))]">
                ...
                {suppliers.length === 0 && (
                  <tr>
                    <td colSpan="4" className="py-8 text-center text-[hsl(var(--muted))]">Belum ada pemasok terdaftar.</td>
                  </tr>
                )}
              </tbody>
            </table>
